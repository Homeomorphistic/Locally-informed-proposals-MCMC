...
\subsection{Definition and basic properties}
	...
	\begin{definition}
		...
		\end{equation*}
		for all indices $l_1< \ldots < l_{p-1} < k < k+m, \; 1 \leq p < k$ and all states $j, i, i_{k-1}, i_{k-2}, \ldots, i_0 \in S$.
	\end{definition}
	...
	\begin{definition}
		A transition matrix in $k$ steps $\PP(k)$ for a MC is a stochastic matrix constructed using transition probabilities:
		\begin{equation*}
	...
	\begin{theorem}
		Let $\bfmu^{(k)} \in \R^N$ be a distribution of a MC at $k$-th step and $\bfmu \in \R^N$ the initial distribution, then for all $k$:
		\begin{equation*}
			\bfmu^{(k)} = \bfmu \PP^k.
		\end{equation*}
	\end{theorem}
	...
	\subsubsection{Irreducibility}
		...
		\begin{definition}[Irreducibility]
			...
			\begin{equation*}
				\forall \; i,j \in S \; \exists \, n \, \PP_{i,j}(k) > 0.
			\end{equation*}
		\end{definition}
